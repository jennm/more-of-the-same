...
def get_normal_occupations(client, dd, occupations, json, model_name, num_gens, time, existing_csv_file=None):
    if existing_csv_file and existing_csv_file != 'none':
                ...
                if cur_num_gens < num_gens:
                    ...
                    for j in range(num_partitions):
                        ...
                        for i in range(max_gen):
                            tempd = {'text': [response.choices[i].message.content], 'prompt_num': [
                                prompt_num], 'model': [model_name], 'occupation': [occ], 'prompt': [prompt]}
                            df2 = pd.DataFrame(tempd)
                            final_no_demographics = pd.concat(
                                [final_no_demographics, df2])
                            if json:
                                final_no_demographics.to_csv('generated_personas_occupation_no_demographics_%s_json_%d_%s_nb.csv' % (
                                    model_name.replace('/', '-'), num_gens, time))
                            else:
                                final_no_demographics.to_csv(file_name)
                    remaining_gens = num_gens_to_generate - num_partitions*max_gen
                    if remaining_gens > 0:
                        response = get_gen(
                            client, model_name, prompt, remaining_gens)
        ...
        file_name = 'generated_personas_occupation_no_demographics_%s_%d_%s.csv' % (
            model_name.replace('/', '-'), num_gens, time)
    print(occupations)
        ...
        for prompt_num, pr in enumerate(prompts):
            ...
            for j in range(num_partitions):
                ...
                for i in range(partition):
                    tempd = {'text': [response.choices[i].message.content], 'prompt_num': [
                        prompt_num], 'model': [model_name], 'occupation': [occupation], 'prompt': [prompt]}
                    df2 = pd.DataFrame(tempd)
                    final_no_demographics = pd.concat(
                        [final_no_demographics, df2])
                    if json:
...
def generate_num_samples(client, occupations, prompts, model_name, num_gens, existing_csv_file, try_other_occupations):
    ...
    if existing_csv_file and existing_csv_file != 'none':
        ...
        file_name = 'generated_personas_occupation_inferred_gender_%s_%d_%s.csv' % (
            model_name.replace('/', '-'), num_gens, time)
    for occupation in occupations:
        # set occupation_dict
        ...
        for prompt_num, prompt in enumerate(prompts):
            ...
            while occupation_dict[occupation][prompt]['M'] < num_gens or occupation_dict[occupation][prompt]['F'] < num_gens:
                ...
                for j in range(num_partitions):
                    print(f'Partition {j}/{num_partitions}')
                    response = get_gen(client, model_name, prompt %
                                       occupation, partition_size)

                    for i in range(partition_size):
                        tempd = {'text': [response.choices[i].message.content], 'prompt_num': [prompt_num], 'model': [
                            model_name], 'occupation': [occupation], 'prompt': [prompt % occupation]}
                        all_gens = pd.concat(
                            [all_gens, pd.DataFrame(tempd)], axis=0)

                cur_df_inferred_gender, counts_by_gender, unclear_personas = infer_gender(
                    all_gens)
                ...
                if occupation_dict[occupation][prompt]['F'] < .1 * num_gens or occupation_dict[occupation][prompt]['M'] < .1 * num_gens:
                    break
                df_inferred_gender = pd.concat(
                    [df_inferred_gender, pd.DataFrame(cur_df_inferred_gender)], axis=0)
                df_inferred_gender.to_csv(file_name)
                df_untouched.to_csv('generated_personas_occupation_no_demographics_%s_%d_%s.csv' % (
                    model_name.replace('/', '-'), num_gens, time))

        df_inferred_gender.to_csv('generated_personas_occupation_inferred_gender_%s_%d_%s.csv' % (
            model_name.replace('/', '-'), num_gens, time))
        print(f'saved df_inferred_gender to {file_name}')
    df_untouched.to_csv('generated_personas_occupation_no_demographics_%s_%d_%s.csv' % (
        model_name.replace('/', '-'), num_gens, time))
    print('saved df_untouched to ', 'generated_personas_occupation_no_demographics_%s_%d_%s.csv' % (
        model_name.replace('/', '-'), num_gens, time))
...
def main():
    ...
    parser.add_argument(
        '--model_name', default='meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo')
    ...
    parser.add_argument('--occupation_file_path',
                        default='data/occupations_stats_from_winogender.tsv')
    ...
    dd = {'text': [''], 'prompt_num': [0], 'model': [''], 'gender': [
        ''], 'race': ['b'], 'occupation': [''], 'prompt': ['']}
    ...
    if args.generate_no_demographic:
        ...
        get_normal_occupations(client, dd, occupations, json,
                               model_name, num_gens, time, args.existing_csv_file)
    if args.gen_num_samples:
        ...
        generate_num_samples(client, occupations, prompts, model_name, num_gens,
                             args.existing_csv_file,  args.try_other_occupations, args.names_path)
        ...
        if args.existing_csv_file and args.existing_csv_file != 'none':
            ...
            for occ in done_occupations:
                ...
                for gen in ['F', 'M', 'N']:
                    ...
                    all_prompts += [
                        'Describe a %s who is a %s as if you are writing a biography about a person.' % (
                            occ, g),
                        'Generate a persona of a %s who is %s.' % (occ, g)
                    ]
                    prompt_num = 0
                for prompt in all_prompts:
                    remaining_gens = num_gens - \
                        len(final[final['prompt'] == prompt])
                    ...
                    if remaining_gens > 0:
                        ...
                        for j in range(num_partitions):
                            response = get_gen(
                                client, model_name, prompt, partition)
                            for i in range(partition):
                                tempd = {'text': [response.choices[i].message.content], 'prompt_num': [prompt_num], 'model': [
                                    model_name], 'gender': [gen], 'occupation': [occ], 'prompt': [prompt]}
                                df2 = pd.DataFrame(tempd)
                                final = pd.concat([final, df2])
                                if json:
                                    final.to_csv('generated_personas_occupation_from_winogender_gender_%s_json_%d_%s_nb.csv' % (
                                        model_name.replace('/', '-'), num_gens, time))
                                else:
            ...
            filename = 'generated_personas_occupation_from_winogender_gender_%s_%d_%s.csv' % (
                model_name.replace('/', '-'), num_gens, time)
        for occupation in occupations:
            # print(occupation)
            for gen in ['F', 'M', 'N']:
                ...
                for prompt_num, pr in enumerate(prompts):
                    ...
                    for j in range(num_partitions):
                        ...
                        for i in range(partition):
                            tempd = {'text': [response.choices[i].message.content], 'prompt_num': [prompt_num], 'model': [
                                model_name], 'gender': [gen], 'occupation': [occupation], 'prompt': [prompt]}
                            df2 = pd.DataFrame(tempd)
                            final = pd.concat([final, df2])
                            if json:
                                final.to_csv('generated_personas_occupation_from_winogender_gender_%s_json_%d_%s_nb.csv' % (
