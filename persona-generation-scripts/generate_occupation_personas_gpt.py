            ...
            while occupation_dict[occupation][prompt]['M'] < num_gens or occupation_dict[occupation][prompt]['F'] < num_gens:
                ...
                for i in range(num_gens):
                    tempd = {'text': [response['choices'][i]['message']['content']], 'prompt_num': [
                        prompt_num], 'model': [model_name], 'occupation': [occupation], 'prompt': [prompt % occupation]}
                    all_gens = pd.concat(
    ...
    for occupation in occupations:
        ...
        for prompt_num, pr in enumerate(prompts):
            ...
            for i in range(num_gens):
                ...
                if json:
                    final_no_demographics.to_csv(
                        'generated_personas_occupation_no_demographics_%s_json_%d_%s_nb.csv' % (model_name, num_gens, time))
                else:
                    final_no_demographics.to_csv(
                        'generated_personas_occupation_no_demographics_%s_%d_%s.csv' % (model_name, num_gens, time))
...
def main():
    ...
    if args.generate_no_demographic:
        ...
        get_normal_occupations(dd, occupations, json,
                               model_name, num_gens, time)
    ...
    if args.gen_num_samples:
        ...
        generate_num_samples(occupations, prompts, model_name, num_gens,
        ...
        for occupation in occupations:
            ...
            for gen in ['F', 'M', 'N']:
                ...
                for prompt_num, pr in enumerate(prompts):
                    ...
                    for i in range(num_gens):
                        tempd = {'text': [response['choices'][i]['message']['content']], 'prompt_num': [
                            prompt_num], 'model': [model_name], 'gender': [gen], 'occupation': [occupation], 'prompt': [prompt]}
                        df2 = pd.DataFrame(tempd)
                        ...
                        if json:
                            ...
                            final.to_csv('generated_personas_occupation_from_winogender_gender_%s_%d_%s.csv' % (
                                model_name, num_gens, time))
